...
containers/control/            Control-plane image: multi-stage build burying the emsdk/AdvantageScope compile, the fabrica dispatching entrypoint
...
V2 is complete. The system uses per-student merged containers (`fabrica-workspace-<id>`) running VSCodium with bundled Java and WPILib extensions. The control plane proxies editor, run, and telemetry traffic through authenticated routes.
...
`fabrica-admin` Legion group, recomputed live on every request — there is
...
`bun run dev:control` uses; **network mode** (`FRC_CONTAINER_NETWORK=fabrica`)
...
(the `fabrica-admin` group — see `docs/decisions/046-legion-auth-integration.md`),
with zero exec steps on this side; ops commands run as `fabrica <subcommand>`
...
- Containerized ops (compose deployments): `docker compose exec control fabrica <subcommand>` (or `docker compose run --rm control <subcommand>` while the plane is stopped) — see `docs/reference/cli-reference.md`
