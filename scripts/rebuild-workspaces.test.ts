...
async function tempDb(): Promise<{ db: Database; dbPath: string }> {
	const dir = await mkdtemp(join(tmpdir(), "scriptum-rebuild-"));
	tempDirs.push(dir);
...
describe("rebuildWorkspaces", () => {
	test("dry-run removes nothing and leaves leases unchanged", async () => {
		...
		try {
			insertLease(db, "ws_a", "running", "scriptum-workspace-a");
			const calls: string[][] = [];
			const dockerRunner: DockerRunner = async (args) => {
				calls.push(args);
				return ok("scriptum-workspace-a\n");
			};
			...
			expect(leases(db)[0]).toMatchObject({
				...
				halsim_port: 34000,
				nt4_port: 25810,
				vscode_container: "scriptum-workspace-a",
				vscode_port: 33000,
			});
		} finally {
			db.close();
	...
	test("removes only Docker containers returned by the managed V2 label query", async () => {
		...
		try {
			...
			expect(calls).toEqual([
				...
				["rm", "-f", "scriptum-workspace-a"],
				["rm", "-f", "scriptum-workspace-b"],
			]);
		} finally {
			db.close();
		}
	...
	test("clears container leases after removing managed V2 containers", async () => {
		...
		try {
			insertLease(db, "ws_a", "running", "scriptum-workspace-a");
			insertLease(db, "ws_b", "stopped", "scriptum-workspace-b");
			const dockerRunner: DockerRunner = async (args) =>
				args[0] === "container"
					? ok("scriptum-workspace-a\nscriptum-workspace-b\n")
					: ok();
