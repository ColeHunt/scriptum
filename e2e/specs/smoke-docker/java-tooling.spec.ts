...
const image =
	...
	`${process.env.FABRICA_IMAGE_NS ?? "ghcr.io/mathewdunne"}/coderunner-workspace:${process.env.FABRICA_TAG ?? "latest"}`;
...
async function waitForWpilibEditorBuild(name: string): Promise<string> {
	return waitFor(
		...
		(evidence) =>
			...
			evidence.includes("FABRICA_ROBOT_CLASS_PRESENT") &&
			evidence.includes("FABRICA_WPILIB_BUILD_IDLE"),
		240_000,
	);
}
