...
The easiest path is to set `SCRIPTUM_ADMIN_EMAIL` in `/opt/coderunner/.env`
...
gcloud compute ssh coderunner --zone=us-central1-a --tunnel-through-iap \
  --command='cd /opt/coderunner && echo "SCRIPTUM_ADMIN_EMAIL=<your-email>" | sudo tee -a .env && sudo docker compose up -d'
```
...
2. `scp`s the compose files to the VM, pins `SCRIPTUM_TAG=<tag>` in
